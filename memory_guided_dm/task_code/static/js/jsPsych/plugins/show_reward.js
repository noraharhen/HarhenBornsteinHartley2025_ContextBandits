...
jsPsych.plugins["show_reward"] = (function () {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      choice_image: {
        type: jsPsych.plugins.parameterType.FUNCTION,
        pretty_name: 'Choice image',
        default: undefined,
        description: 'Show chosen pirate.'
      },
      reward: {
        type: jsPsych.plugins.parameterType.FUNCTION,
        pretty_name: 'Reward image',
        default: undefined,
        description: 'Show reward.'
      }
    }
  }
  ...
  plugin.trial = function (display_element, trial) {
    ...
    chosen_pirate = new Image();
    chosen_pirate.src = trial.choice_image;

    chosen_pirate.onload = function(){
      c_ctx.drawImage(chosen_pirate, c.width / 2 - (chosen_pirate.width*0.6) / 2,
      c.height - (chosen_pirate.height*0.6)/1.0,chosen_pirate.width*0.6, chosen_pirate.height*0.6);
    }

    reward = new Image();

    if (trial.reward == 1) {
      reward.src = '../../make_stim/stims-10.png';
    } else if (trial.reward == 0) {
      reward.src = '../../make_stim/stims-11.png';}

    reward.onload = function(){
      c_ctx.drawImage(reward, c.width / 2 - (reward.width*0.6) / 2,
      c.height - (reward.height*0.6)/1.0,reward.width*0.6, reward.height*0.6);
    }
  }
